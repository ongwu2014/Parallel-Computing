...
int main(int argc, char* argv[]) {
  ...
  for (stepCounter=0; stepCounter < NumGenerations; stepCounter++)
  {
       ...
       /* send to next proc, recv from previous */
       if (myid%2 == 0)
       {
          if(myid < numprocs-1) MPI_Send(uOld[Nlocal],N+2, MPI_INT, myid+1, 0, MPI_COMM_WORLD);
          if(myid > 0) MPI_Recv(uOld[0],N+2, MPI_INT, myid-1, 0, MPI_COMM_WORLD,&status1);
       }
       else
       {
          if(myid > 0) MPI_Recv(uOld[0],N+2, MPI_INT, myid-1, 0, MPI_COMM_WORLD,&status1);
       ...
       /* send to previous proc, recv from next */
       if (myid%2 == 0)
       {
          if(myid > 0) MPI_Send(uOld[1],N+2, MPI_INT, myid-1, 0, MPI_COMM_WORLD);
          if(myid < numprocs-1) MPI_Recv(uOld[Nlocal+1],N+2, MPI_INT, myid+1, 0, MPI_COMM_WORLD,&status2);
       }
       else
       {
          if(myid < numprocs-1) MPI_Recv(uOld[Nlocal+1],N+2, MPI_INT, myid+1, 0, MPI_COMM_WORLD,&status2);
